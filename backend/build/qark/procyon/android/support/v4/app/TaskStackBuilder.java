...
public class TaskStackBuilder implements Iterable<Intent>
{
    ...
    public TaskStackBuilder addParentStack(final Activity activity) {
        ...
        Intent element = NavUtils.getParentActivityIntent(activity);
        while (element != null) {
            this.mIntents.add(size, element);
            try {
                element = NavUtils.getParentActivityIntent((Context)activity, element.getComponent());
                continue;
            }
    ...
    public TaskStackBuilder addParentStack(final Class<?> clazz) {
        final int size = this.mIntents.size();
        try {
            for (Intent element = NavUtils.getParentActivityIntent(this.mSourceContext, clazz); element != null; element = NavUtils.getParentActivityIntent(this.mSourceContext, element.getComponent())) {
                this.mIntents.add(size, element);
            }
        }
        catch (PackageManager$NameNotFoundException cause) {
    ...
    public void startActivities(final Bundle bundle) {
        ...
        if (!ContextCompat.startActivities(this.mSourceContext, array, bundle)) {
            final Intent intent = array[array.length - 1];
