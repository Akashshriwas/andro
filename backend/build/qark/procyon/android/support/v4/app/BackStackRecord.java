            ...
            for (Op op = this.mHead; op != null; op = op.next) {
                ...
                if (op.removed != null) {
                    for (int j = op.removed.size() - 1; j >= 0; --j) {
                        ...
                        if (FragmentManagerImpl.DEBUG) {
                            Log.v("BackStackEntry", "Bump nesting of " + obj + " to " + obj.mBackStackNesting);
                        }
                    }
                }
            }
    ...
    public void run() {
        ...
        for (Op op = this.mHead; op != null; op = op.next) {
            switch (op.cmd) {
                ...
                case 2: {
                    Fragment fragment2;
                    Fragment obj = fragment2 = op.fragment;
                    if (this.mManager.mAdded != null) {
                        int index = 0;
                        while (true) {
                            fragment2 = obj;
                            if (index >= this.mManager.mAdded.size()) {
                                break;
                            }
                            final Fragment obj2 = this.mManager.mAdded.get(index);
                            if (FragmentManagerImpl.DEBUG) {
                                Log.v("BackStackEntry", "OP_REPLACE: adding=" + obj + " old=" + obj2);
                            }
                            Fragment fragment3 = null;
                            Label_0306: {
                                if (obj != null) {
                                    fragment3 = obj;
                                    if (obj2.mContainerId != obj.mContainerId) {
                                        break Label_0306;
                                    }
                                }
                                if (obj2 == obj) {
                                    ...
                                    op.removed.add(obj2);
                                    obj2.mNextAnim = op.exitAnim;
                                    if (this.mAddToBackStack) {
                                        ++obj2.mBackStackNesting;
                                        if (FragmentManagerImpl.DEBUG) {
                                            Log.v("BackStackEntry", "Bump nesting of " + obj2 + " to " + obj2.mBackStackNesting);
                                        }
                                    }
