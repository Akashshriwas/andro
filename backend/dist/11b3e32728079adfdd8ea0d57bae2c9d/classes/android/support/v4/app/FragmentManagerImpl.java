  ...
  extends FragmentManager
{
          ...
          while (j < mActive.size())
          {
            ...
            if (localFragment != null)
            {
              ...
              if (mLoaderManager != null) {
                m = i | mLoaderManager.hasRunningLoaders();
              }
            }
            j++;
            i = m;
  ...
  boolean popBackStackState(Handler paramHandler, String paramString, int paramInt1, int paramInt2)
  {
    ...
    for (;;)
    {
      ...
      if (paramInt1 <= paramInt2)
      {
        ...
        {
          paramHandler.popFromBackStack(bool2);
          paramInt1++;
          break;
        }
      }
  ...
  Parcelable saveAllState()
  {
    ...
    for (;;)
    {
      ...
      if (k < i)
      {
        localObject3 = (Fragment)mActive.get(k);
        int m;
        if (localObject3 != null)
        {
          if (mIndex < 0)
          {
            localObject2 = "Failure saving state: active " + localObject3 + " has cleared index: " + mIndex;
          ...
          m = 1;
          localObject2 = new FragmentState((Fragment)localObject3);
          arrayOfFragmentState[k] = localObject2;
          if ((mState <= 0) || (mSavedFragmentState != null)) {
            break label432;
          }
          mSavedFragmentState = saveFragmentBasicState((Fragment)localObject3);
          if (mTarget != null)
          {
            if (mTarget.mIndex < 0)
            {
              localObject2 = "Failure saving state: " + localObject3 + " has target not in fragment manager: " + mTarget;
              Log.e("FragmentManager", (String)localObject2);
              dump("  ", null, new PrintWriter(new LogWriter("FragmentManager")), new String[0]);
              throw new IllegalStateException((String)localObject2);
            }
        ...
        for (;;)
        {
          ...
          if (DEBUG)
          {
            Log.v("FragmentManager", "Saved state of " + localObject3 + ": " + mSavedFragmentState);
            j = m;
          }
          k++;
          break;
